...
!     INCLUDE 'mpif.h'
      ...
      INTEGER chiffre, nodexchiffre, msgLength, inID
      INTEGER gauss_sum
...
! master receives hostnames
         ...
         do inID = 1, nprocs - 1
            call MPI_Recv (msgLength, 1, MPI_INTEGER,  inID,inID, &
                          MPI_COMM_WORLD, istat, ierr)
            call MPI_Recv (msg,msgLength,MPI_CHARACTER,inID,inID+nprocs, &
                           MPI_COMM_WORLD, istat, ierr)
...
! slaves send
      if (myid .ne. 0) then
         ...
         call MPI_Send (msgLength, 1, MPI_INTEGER,     0, myid+2*nprocs, &
                        MPI_COMM_WORLD, ierr)   ! Send msgLength
         call MPI_Send (msg, msgLength, MPI_CHARACTER, 0, myid+3*nprocs, &
                        MPI_COMM_WORLD, ierr)   ! Send msg
         call MPI_Send (nodexchiffre, 1, MPI_INTEGER,    0, myid+4*nprocs, &
                        MPI_COMM_WORLD, ierr)   ! Send result
      else
! master receives
         do inID = 1, nprocs - 1
            call MPI_Recv (msgLength, 1, MPI_INTEGER,inID,inID+2*nprocs, &
                           MPI_COMM_WORLD, istat, ierr)
            call MPI_Recv (msg,msgLength,MPI_CHARACTER,inID,inID+3*nprocs, &
                           MPI_COMM_WORLD, istat, ierr)
            call MPI_Recv (nodexchiffre,1,MPI_INTEGER, inID,inID+4*nprocs, &
                           MPI_COMM_WORLD, istat, ierr)
         enddo
      endif
    endif ! MultiProcessorRun 
